...
/**
 * Create the ActionsCard class with loaded dependencies
 * @param {Object} dependencies - Loaded LitElement dependencies
 * @returns {Class} ActionsCard class
 */
export function createActionsCard() {
  const { LitElement, html, css, fireEvent } = getDependencies();
  ...
  class ActionsCard extends LitElement {
    ...
    setConfig(config) {
      ...
      if (config && config.card) {
        ...
        if (
          !this._childCard ||
          JSON.stringify(this._currentCardConfig) !==
            JSON.stringify(config.card)
        ) {
    ...
    async _createCardElement(cardConfig) {
      ...
      try {
        const actualCardConfig = Array.isArray(cardConfig)
          ? cardConfig[0]
          : cardConfig;
    ...
    async _finalizeCardElement(element, cardType) {
      ...
      logDebug(
        "INIT",
        "Child card created/updated using card helpers:",
        cardType,
      );
    ...
    async _createCardElementDirect(cardConfig, retryCount = 0) {
      ...
      if (retryCount > UI_CONSTANTS.MAX_CARD_CREATION_RETRIES) {
        logDebug(
          "ERROR",
          `Failed to create card element after 10 retries: ${cardType}`,
        );
      ...
      if (typeof element.setConfig !== "function") {
        if (retryCount < 5) {
          ...
          logDebug(
            "ERROR",
            `Element ${elementTag} does not have setConfig method after 5 retries`,
          );
          this._createErrorCard(
            `Card type ${cardType} is not available`,
            cardConfig,
          );
          return;
        }
      ...
      try {
        ...
        this._createErrorCard(
          `Error setting up card: ${error.message}`,
          cardConfig,
        );
      }
    }
    ...
    _preventDefaultDialogs() {
      ...
      const setupPrevention = () => {
        if (this._childCard) {
          ...
          eventTypes.forEach((eventType) => {
            // Add to main element
            this._childCard.addEventListener(
              eventType,
            ...
            if (this._childCard.shadowRoot) {
              this._childCard.shadowRoot.addEventListener(
                eventType,
                this._childCardClickHandler,
                {
                  capture: true,
      ...
      const trySetup = (attempts = 0) => {
        ...
        try {
          ...
          logDebug(
            "INIT",
            `Dialog prevention setup attempt ${attempts + 1} failed, retrying...`,
          );
    ...
    _cleanupDefaultDialogPrevention() {
      if (this._childCard && this._childCardClickHandler) {
        ...
        eventTypes.forEach((eventType) => {
          this._childCard.removeEventListener(
            eventType,
            this._childCardClickHandler,
            true,
          );
    ...
    getCardSize() {
      ...
      if (
        this._childCard &&
        typeof this._childCard.getCardSize === "function"
      ) {
        try {
    ...
    _onPointerDown(ev) {
      ...
      while (targetElement && targetElement !== this) {
        if (
          ...
          logDebug(
            "ACTION",
            "Pointer down ignored (interactive element):",
            targetElement.tagName,
          );
          return;
        }
        targetElement =
          targetElement.parentElement || targetElement.getRootNode().host;
      }
      ...
      if (
        ...
        this._holdTimeout = window.setTimeout(() => {
          ...
          if (
            Math.abs(ev.clientX - this._startX) < 10 &&
            Math.abs(ev.clientY - this._startY) < 10
          ) {
    ...
    _onPointerUp(ev) {
      ...
      if (this._holdTriggered || moved) {
        ...
        logDebug(
          "ACTION",
          "Tap canceled (hold already triggered or moved too much)",
        );
        this._resetState();
        return;
      }
        ...
        if (this._clickCount === 1) {
          ...
          this._clickTimeout = window.setTimeout(() => {
            ...
            if (this._clickCount === 1 && !this._holdTriggered) {
              logDebug(
                "ACTION",
                "Single tap confirmed (after double-tap timeout)",
              );
    ...
    render() {
      ...
      return html`
        <div
          ...
          style="cursor: ${hasActions
            ? "pointer"
            : "default"}; display: block; height: 100%;"
