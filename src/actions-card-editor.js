 ...
 * @returns {Class} ActionsCardEditor class
 */
export function createActionsCardEditor() {
  ...
  class ActionsCardEditor extends LitElement {
    ...
    async connectedCallback() {
      ...
      while (parent) {
        ...
        parent =
          parent.parentNode ||
          (parent.getRootNode && parent.getRootNode().host);
      }
    ...
    async _ensureComponentsLoaded() {
      ...
      if (!customElements.get("hui-card-picker")) {
        logDebug(
          "EDITOR",
          "Primary loading failed, trying alternative approach",
        );
      ...
      if (!customElements.get("hui-card-picker")) {
        logDebug(
          "EDITOR",
          "All loading attempts failed, card picker may not be available",
        );
      } else {
    ...
    _setupGlobalHandlers() {
      ...
      this._globalEventHandler = (e) => {
        ...
        if (e.target && this.shadowRoot && this.shadowRoot.contains(e.target)) {
          ...
          logDebug(
            "EVENT",
            `Marking ${e.type} event as processed by actions card editor`,
          );
        ...
        if (e.type === "keydown" || e.type === "input") {
          ...
          if (isSearchInput) {
            ...
            logDebug(
              "EVENT",
              `Marking ${e.type} search input event as processed`,
            );
    ...
    _valueChanged(ev) {
      ...
      const key =
        target.configAttribute || target.getAttribute("data-attribute");
      ...
      if (option && option.includes("_action")) {
        ...
        if (key) {
          ...
          const cleanedConfig = this._cleanActionConfig(
            actionConfig,
            newActionType,
            option,
          );
    ...
    _fireConfigChanged() {
      ...
      logDebug(
        "EVENT",
        "Firing config-changed event with config:",
        this._config,
      );
      this.dispatchEvent(event);
    }
      ...
      try {
        ...
        const dialogParams = {
          ...
          saveCardConfig: async (savedCardConfig) => {
            ...
            logDebug(
              "CONFIG",
              "Updating wrapped card with saved config:",
              savedCardConfig,
            );
            this._config = {
              ...this._config,
    ...
    _shouldShowCardPicker() {
      ...
      const shouldShow =
        noCard && (showAfterRemoval || hasConfigButNoCard || !this._config);
    ...
    render() {
      ...
      return html`
        <div
          class="card-config"
          @keydown=${(e) => (e._processedByActionsCardEditor = true)}
        >
    ...
    _renderActionsConfiguration() {
      ...
      return html`
        <div class="section">
          ...
          <div class="action-container">
            ${this._renderActionTypeDropdown(
              "double_tap_action",
              doubleTapAction,
            )}
            ${this._renderActionDetails("double_tap_action", doubleTapAction)}
            ${this._renderConfirmationConfig(
              "double_tap_action",
              doubleTapAction,
            )}
          </div>
