    ...
    try {
        ...
        for (const source of cdnSources) {
            try {
                ...
                console.warn(`Actions Card: Failed to load from ${source}:`, e);
            }
        }
        ...
        for (const source of helperSources) {
            try {
                ...
                console.warn(`Actions Card: Failed to load helpers from ${source}:`, e);
            }
        }
        ...
        console.error('Actions Card: Failed to load dependencies:', error);
        return false;
    }
}

// Load dependencies and initialize card
loadDependencies().then(success => {
    if (!success) {
        // Create error element
        customElements.define('actions-card', class extends HTMLElement {
            setConfig() {
                throw new Error('Actions Card: Failed to load required dependencies. Please check your internet connection or consider using a bundled version for offline support.');
            }
        });
        return;
    }
